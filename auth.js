...
verifyBtn.onclick = async () => {
    ...
    const trimmedJoinCode = joinCodeEntered.trim();

    if (!trimmedJoinCode || trimmedJoinCode.length === 0) {
        showBootstrapAlert("Please enter a valid join code");
    ...
    let inviteSnap;
    try {
        inviteRef = doc(db, 'invites', trimmedJoinCode);
        ...
        stepCode.classList.remove('d-none');
        verifyBtn.disabled = false;
        showBootstrapAlert("Failed to verify join code: " + error.message);
        return;
    }
...
signupForm.addEventListener('submit', async (e) => {
    ...
    loading.classList.remove('d-none');
    loading.classList.add('d-flex');
    stepCreds.classList.add('d-none');
    ...
    try {
        const inviteRef = doc(db, 'invites', trimmedJoinCode);
